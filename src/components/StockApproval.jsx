...
export default function StockApproval() {
  ...
  // Supabase Table Name
  const INDENT_TABLE = "INDENT-PO"

  // Mapping of Supabase column names to the expected internal "colX" keys
  const COLUMN_MAPPING = [
    "Timestamp", "Indent Id.", "Firm Name", "Generated By", "Vendor", "Material", 
    "Quantity", "Current Stock As Per factory", "Priority", "Delivery Order No.", 
    "Notes", "Planned1", "Actual1", "Delay1", "Approved Qty", "Approval Status", 
    "Remarks", "Planned2", "Actual2", "Delay2", "Have To Make PO", "Rate", 
    "Lead Time To Lift (days)", "Total Quantity", "Total Amount", "PO Copy", 
    "Advance To Be Paid", "To Be Paid Amount", "When To Be Paid Amount", "PO Notes"
  ];
  ...
  useEffect(() => {
    const fetchIndentsData = async () => {
      ...
      try {
        ...
        // Generate column metadata similar to how Google Sheets headers were handled
        const generatedAllColumnsMeta = COLUMN_MAPPING.map((colName, index) => {
          const colId = `col${index}`
          const friendlyHeader =
            Object.keys(FRIENDLY_HEADER_TO_DATA_KEY_MAP).find(
              (key) => FRIENDLY_HEADER_TO_DATA_KEY_MAP[key] === colId,
            ) || colName
            
          return {
            header: friendlyHeader,
            dataKey: colId,
            toggleable: Object.values(FRIENDLY_HEADER_TO_DATA_KEY_MAP).includes(colId),
            alwaysVisible: colId === "col1",
            isApproveUI: false,
            isHistoryUI: false,
          }
        })
        ...
        let processedIndents = rowsData.map((row) => {
          const rowObject = {
            ...
            id: (row["Indent Id."] || "").trim(),
          }

          // Map the row data to the internal colX structure for compatibility
          COLUMN_MAPPING.forEach((colName, index) => {
            rowObject[`col${index}`] = row[colName] !== null && row[colName] !== undefined ? String(row[colName]) : ""
          })

          // Add convenience properties used throughout the component
          return {
            ...rowObject,
            timestampOriginal: rowObject.col0 || "",
            generatedBy: rowObject.col3 || "",
            vendorName: rowObject.col4 || "",
            rawMaterialName: rowObject.col5 || "",
            quantity: rowObject.col6 || "",
            currentStock: rowObject.col7 || "",
            priority: rowObject.col8 || "",
            deliveryOrderNo: rowObject.col9 || "",
            notes: rowObject.col10 || "",
            columnL_Check: rowObject.col11 || "",
            columnM_Check: rowObject.col12 || "",
            approvedQty_original: rowObject.col14 || "",
            status_original: rowObject.col15 || "",
            remark_original: cleanRemark(rowObject.col16 || ""),
          }
        })
  ...
  async function handleSubmitSelected(e) {
    ...
    // Use a sequential for...of loop for clearer error handling
    for (const indentId of selectedIndentIds) {
      const indent = indents.find(item => item.id === indentId);
      if (!indent || !indent._dbId) {
        errorMessages.push(`Could not find database record for indent ${indentId}.`);
        continue;
      }
      ...
      try {
        ...
        const updates = {
          "Actual1": timestamp,
          "Approved Qty": parseFloat(approvedQtys[indent.id]) || parseFloat(indent.quantity),
                    ...
                    {processedIndents.length === 0 ? (
                      ...
                      <div className="overflow-x-auto rounded-lg border border-gray-200 max-h-[calc(100vh-500px)]">
                        <Table className="w-full text-sm">
                          ...
                          <TableBody className="bg-white divide-y divide-gray-200">
                            {processedIndents.map((indent, index) => (
                              <TableRow key={indent.id || `history-${index}`} className="hover:bg-gray-50">
                                {getHistoryTableColumns.map((col) => (
                                  <TableCell key={col.dataKey} className="px-4 py-1.5">
                                    {col.dataKey === "col12"

...
export default function IndentForm() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const extractUniqueValues = (data, columnName) => {
        ...
        const values = data.map((item) => item[columnName] ? String(item[columnName]).trim() : "").filter(Boolean)
        return [...new Set(values)].sort()
      }

      const options = {
        generatedBy: extractUniqueValues(masterData, "Generated By"),
        vendorName: extractUniqueValues(masterData, "Vendor Name"),
        firmName: extractUniqueValues(masterData, "Firm Name"),
        rawMaterialName: extractUniqueValues(masterData, "Raw Material Name"),
      }
      ...
      // 2. Fetch the latest Indent ID to calculate next RL number
      const { data: latestIndent, error: indentError } = await supabase
        .from(INDENT_TABLE)
        .select('"Indent Id."')
      ...
      let highestNumber = 0
      if (latestIndent && latestIndent.length > 0) {
        const idValue = latestIndent[0]["Indent Id."]
        if (idValue && idValue.startsWith("RL-")) {
          const num = parseInt(idValue.substring(3), 10)
          if (!isNaN(num)) highestNumber = num
        }
      }
  ...
  }, [INDENT_TABLE, MASTER_TABLE, user, allowedSteps])

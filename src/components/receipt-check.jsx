      ...
      try {
        ...
        let processedData = rows.map((row) => {
          const rowData = { 
            ...
            rawMaterialName: row["Raw Material Name"] || "",
            billNo: row["Bill No."] || "",
            qty: row["Qty"] || "",
            type: row["Type"] || "",
            billCopy: row["Bill Image"] || "",
            driverNo: row["Driver No."] || "",
            areaLifting: row["Area lifting"] || "",
            truckNo: row["Truck No."] || "",
            plannedDate_formatted: row["Planned 1"] || "",
            filterColT_val: row["Planned 1"] || "",
            filterColU_val: row["Actual 1"] || "",
            actual1Timestamp: row["Actual 1"] || "",
            dateOfReceiving_fromSheet: row["Date Of Receiving"] || "",
            dateOfReceiving_formatted: row["Date Of Receiving"] || "",
            totalBillQuantity_fromSheet: row["Total Bill Quantity"] || "",
            actualQuantity_fromSheet: row["Actual Quantity"] || "",
            physicalCondition_fromSheet: row["Physical Condition"] || "",
            moisture_fromSheet: row["Moisture"] || "",
            physicalImageUrl_fromSheet: row["Physical Image Of Product"] || "",
            weightSlipImageUrl_fromSheet: row["Image Of Weight Slip"] || "",
            weightSlipQty_fromSheet: row["Weight Slip Qty"] || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatePayload = {
        ...
        "Total Bill Quantity": parseFloat(formData.totalBillQuantity) || 0,
        "Actual Quantity": parseFloat(formData.actualQuantity) || 0,
        "Physical Condition": formData.physicalCondition,
        "Moisture": parseFloat(formData.moisture) || 0,
        "Physical Image Of Product": physicalImageUrl,
        "Image Of Weight Slip": weightSlipImageUrl,
        "Weight Slip Qty": parseFloat(formData.weightSlipQty) || 0,
      };
      ...
      // --- Pushing to 'accounts' table for Step 7: Audit ---
      const accountsPayload = {
        "Timestamp": timestamp,
        "Lift Number": selectedLift.liftNo,
        "Type": selectedLift.type,
        ...
        "Area Lifting": selectedLift.areaLifting,
        "Truck No.": selectedLift.truckNo,
        "Transporter Name": "N/A",
        "Bill Image": selectedLift.billCopy,
        "Bilty No.": selectedLift.id,
        "Type Of Rate": "N/A",
        "Rate": 0,
        "Truck Qty": parseFloat(formData.totalBillQuantity) || 0,
        "Bilty Image": selectedLift.billCopy,
        "Qty Difference Status": (parseFloat(formData.totalBillQuantity) === parseFloat(formData.actualQuantity)) ? 'Matched' : 'Mismatch',
        "Difference Qty": Math.abs((parseFloat(formData.totalBillQuantity) || 0) - (parseFloat(formData.actualQuantity) || 0)),
        "Weight Slip": weightSlipImageUrl,
        "Total Freight": 0
      };
